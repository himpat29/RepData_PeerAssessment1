...
author: "Himanshu Shekhar Pathak"
date: "26 April 2019"
...
```{r, echo = TRUE}
setwd("C:/Users/HIMANSHU/Desktop/R template/reproducible analysis")
...
G <-ggplot(Q4,aes(x=Q4$date,y=Q4$`Mean Steps`))+geom_bar(stat="identity")+scale_x_date()
G <- G + ylab("Mean StepsEveryday")+xlab("Date")+ggtitle("MeanSteps by Date")
png("plot4.png")
print(G)
dev.off()
```
...
png("plot8.png")
G <- ggplot(data=Q8.1,aes(y=Steps,x=interval,group=1,color=WDWE))+geom_line() 
G <- G +scale_x_discrete(breaks = seq(0, 2500, by = 300))+ylab("Mean Steps")+xlab("Intervals")
G <- G +ggtitle("Mean steps across intervals by Weekend and Weekday")
dev.off()
print(G)
